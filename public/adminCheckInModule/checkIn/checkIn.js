...
// Set Date
if (date) $('#date').text(moment(date).format('MMMM DD, YYYY'))
else $('#date').text(moment().format('MMMM DD, YYYY'))
...
if (from && to && time) {
  ...
  $('#to-location').val(to)
  $('#time-select').append(
    `<option value="${time}">${moment(time, 'HH:mm').format(
      'hh:mm A'
    )}</option>`
  )
  ...
  if ((slotCount, reservedCount, presentCount, walkInCount)) {
    $('#reserved-seats').html(
      `<span class="bold">${reservedCount}</span> out of ${slotCount} `
    )
    $('#currently-present').html(
      `<span class="bold">${presentCount}</span> out of ${reservedCount} `
    )
    $('#walk-in').html(
      `<span class="bold">${walkInCount}</span> out of ${slotCount} `
    )
    $('.information-container').show()
    $('.information-container').css('display', 'flex')
  }
  ...
  if (passengerList.length === 0) {
    $('#passenger-list-container').append(
      '<div class="no-passenger">No Reserved Passenger</div>'
    )
  }
}
...
if (passengerList.length === 0 && !from && !to && !time)
  $('#passenger-list-container').append(
    '<div class="no-passenger">Select a trip and time for this date to view list of reservations</div>'
  )
...
function changeDate(action) {
  ...
  if (action === 'prev')
    window.location.href = `/admin/checkin?date=${moment(dateNow)
      .subtract(1, 'days')
      .format('YYYY-MM-DD')}`
  else if (action === 'next')
    window.location.href = `/admin/checkin?date=${moment(dateNow)
...
async function handleLocationChange() {
  if (checkLocationIsSelected()) {
    const res = await fetch(
      `/admin/checkin/time?date=${moment(date).format('YYYY-MM-DD')}&from=${$(
        '#from-location'
      ).val()}&to=${$('#to-location').val()}`
    ...
    if (res.success) {
      ...
      $('#time-select').empty()
      $('#time-select').append(
        `<option disabled selected value>No Available Time</option>`
      )
    }
  }
}
...
$('#time-select').change(() => {
  window.location.href = `/admin/checkin?date=${date}&from=${$(
    '#from-location'
  ).val()}&to=${$('#to-location').val()}&time=${$('#time-select').val()}`
})
...
$('#search').on('keyup', function () {
  ...
  const result = passengerList.filter(
    (passenger) =>
      passenger.name.toLowerCase().includes($(this).val().toLowerCase()) ||
      passenger.id.toString().includes($(this).val())
  )
