...
scheduleInformation = JSON.parse(scheduleInformation.replaceAll("&#34;", '"')).schedules
...
const dateDisplay = moment(from).format('MMM D') + '-' + moment(to).format('MMM D');
...
$('#date-range').on('apply.daterangepicker', function(ev, picker) {
  ...
  const dateDisplay = moment(startDate).format('MMM D') + '-' + moment(endDate).format('MMM D');
...
function deleteTime(from, to, time, isWeekday, isLocal) {
  if(!isLocal) {
    ...
    if(isWeekday === 'true')
      editInformation.addedTime[pointer].weekdays = editInformation.addedTime[pointer].weekdays.filter(e => e.time !== time)
    else 
      editInformation.addedTime[pointer].saturdays = editInformation.addedTime[pointer].saturdays.filter(e => e.time !== time)
  }
  let id = ('#' + from.replaceAll(" ", "") + time.replace(':', '\\:')).toString();
...
const fromTime = scheduleInformation.filter(schedule => schedule.from === fromName);
const toTime = scheduleInformation.filter(schedule => schedule.from === toName);
// weekdays
$('#weekdays-from-time-container').append(
  fromTime[0].weekdays.map((time24) => {
    let time12 = moment(time24, 'HH:mm').format('hh:mm A')
    return (
      '<div id="'+ fromName.replaceAll(" ", "") + time24 + 'weekday">'
      + time12
      + '<button onclick="deleteTime(\''+ fromName +'\', \''+ toName + '\' , \'' + time24 + '\', \'true\')">Delete</button>' +
      '</div>'
    )
  })
)
...
$('#saturdays-from-time-container').append(
  fromTime[0].saturdays.map((time24) => {
    let time12 = moment(time24, 'HH:mm').format('hh:mm A')
    return (
      '<div id="' + fromName.replaceAll(" ", "") + time24 + 'saturday">'
      + time12
      + '<button onclick="deleteTime(\''+ fromName +'\', \''+ toName + '\' , \'' + time24 + '\', \'false\')">Delete</button>' +
      '</div>'
    )
  })
)
...
$('#weekdays-to-time-container').append(
  toTime[0].weekdays.map((time24) => {
    let time12 = moment(time24, 'HH:mm').format('hh:mm A')
    return (
      '<div id="' + toName.replaceAll(" ", "") + time24 + 'weekday">'
      + time12
      + '<button onclick="deleteTime(\''+ toName +'\', \''+ fromName + '\' , \'' + time24 + '\', \'true\')">Delete</button>' +
      '</div>'
    )
  })
)
...
$('#saturdays-to-time-container').append(
  toTime[0].saturdays.map((time24) => {
    let time12 = moment(time24, 'HH:mm').format('hh:mm A')
    return (
      '<div id="' + toName.replaceAll(" ", "") + time24 + 'saturday">'
      + time12
      + '<button onclick="deleteTime(\''+ toName +'\', \''+ fromName + '\' , \'' + time24 + '\', \'false\')">Delete</button>' +
      '</div>'
    )
  })
)
...
$('#weekdays-from-time-container').on('change', '#weekdays-from-time-input', function() {
  ...
  $('#weekdays-from-time-container').append(
    '<div id="' + fromName.replaceAll(" ", "") + time24 + 'weekday">'
      + time12
      + '<button onclick="deleteTime(\''+ fromName +'\', \''+ toName + '\' , \'' + time24 + '\', \'true\', \'true\')">Delete</button>' +
    '</div>'
  )
  $('#weekdays-from-time-input').remove()
  editInformation.addedTime[0].weekdays.push({
    time: time24,
  })
})
// saturday
...
$('#saturdays-from-time-container').on('change', '#saturdays-from-time-input', function() {
  ...
  $('#saturdays-from-time-container').append(
    '<div id="' + fromName.replaceAll(" ", "") + time24 + 'saturday">'
      + time12
      + '<button onclick="deleteTime(\''+ fromName +'\', \''+ toName + '\' , \'' + time24 + '\', \'false\', \'true\')">Delete</button>' +
    '</div>'
  )
  $('#saturdays-from-time-input').remove()
  editInformation.addedTime[0].saturdays.push({
    time: time24,
  })
})
...
$('#weekdays-to-time-container').on('change', '#weekdays-to-time-input', function() {
  ...
  $('#weekdays-to-time-container').append(
    '<div id="' + toName.replaceAll(" ", "") + time24 + 'weekday">'
      + time12
      + '<button onclick="deleteTime(\''+ toName +'\', \''+ fromName + '\' , \'' + time24 + '\', \'true\', \'true\')">Delete</button>' +
    '</div>'
  )
  $('#weekdays-to-time-input').remove()
  editInformation.addedTime[1].weekdays.push({
    time: time24,
  })
})
// saturday
...
$('#saturdays-to-time-container').on('change', '#saturdays-to-time-input', function() {
  ...
  $('#saturdays-to-time-container').append(
    '<div id="' + toName.replaceAll(" ", "") + time24 + 'saturday">'
      + time12
      + '<button onclick="deleteTime(\''+ toName +'\', \''+ fromName + '\' , \'' + time24 + '\', \'false\', \'true\')">Delete</button>' +
    '</div>'
  )
  $('#saturdays-to-time-input').remove()
  editInformation.addedTime[1].saturdays.push({
    time: time24,
  })
})
